...
if (isset($_POST["ver_cf"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $userExists = operatorUserExists($conn, $id_utente, $id_operatore);
        if ($userExists === false) {
...
if (isset($_POST["oper"])) {
   ...
   //Gestione dell'utente inserito più volte se ricarichi questa pagina.
    $userExists = userExists($_POST['CodiceFiscale']);
    if ( $userExists === false) {
        $stmt = $conn->prepare($sql);
...
foreach ($anagrafica as $value) {
    ...
    if ($tipo == "lista") {
        ...
        $tmp = ($value["name"] == "CodiceFiscale") ? (isset($_POST["ver_cf"]) ? $_POST["ver_cf"] : "") : "";
        echo "
            <div class=\"m-1 m-md-2 row g-md-2 align-items-center\">
                <div class=\"col-md-2 text-md-end\">
                    <label for=\"nick\" class=\"col-form-label\">{$value["etichetta"]}</label>
